...
public class Beats2Codec extends AbstractCodec {
    ...
    private Message parseEvent(JsonNode event) {
        ...
        final String xfcatlgSourceCollector = event.path(Message.FIELD_XFCATLG_SOURCE_COLLECTOR).asText();
        if (!xfcatlgSourceCollector.isEmpty()) {
            gelfMessage.addField(Message.FIELD_XFCATLG_SOURCE_COLLECTOR, xfcatlgSourceCollector);
        }
        ...
        if (event.isObject()) {
            ...
            onode.remove(Message.FIELD_XFCATLG_SOURCE_COLLECTOR);
        }
