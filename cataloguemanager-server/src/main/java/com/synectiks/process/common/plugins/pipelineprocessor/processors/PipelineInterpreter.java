...
import com.synectiks.process.common.plugins.pipelineprocessor.processors.listeners.RuleMetricsListener;
...
public class PipelineInterpreter implements MessageProcessor {
    ...
    private void appendProcessingError(Rule rule, Message message, String errorString) {
        ...
        if (message.hasField(Message.FIELD_XFCATLG_PROCESSING_ERROR)) {
            message.addField(Message.FIELD_XFCATLG_PROCESSING_ERROR, message.getFieldAs(String.class, Message.FIELD_XFCATLG_PROCESSING_ERROR) + "," + msg);
        } else {
            message.addField(Message.FIELD_XFCATLG_PROCESSING_ERROR, msg);
        }
    }
