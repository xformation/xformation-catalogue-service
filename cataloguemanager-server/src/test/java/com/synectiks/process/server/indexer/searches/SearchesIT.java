...
import com.synectiks.process.server.indexer.ranges.IndexRangeComparator;
...
public abstract class SearchesIT extends ElasticsearchBaseTest {
    ...
    public void determineAffectedIndicesWithRangesExcludeEvents() throws Exception {
        final Set<IndexSet> eventIndexSets = Arrays.asList("xfcatlg-events", "xfcatlg-system-events").stream().
        ...
        final DateTime now = DateTime.now(DateTimeZone.UTC);
        final MongoIndexRange indexRange0 = MongoIndexRange.create("xfcatlg-events_0", now, now.plusDays(1), now, 0);
        final MongoIndexRange indexRange1 = MongoIndexRange.create("xfcatlg-system-events_2", now.plusDays(1), now.plusDays(2), now, 0);
