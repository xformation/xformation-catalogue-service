...
public class RoleServiceImpl implements RoleService {
    ...
    public RoleServiceImpl(MongoConnection mongoConnection,
        ...
        adminRoleObjectId = checkNotNull(ensureBuiltinRole(ADMIN_ROLENAME, Sets.newHashSet("*"), "Admin",
                                                           "Grants all permissions for cataloguemanager administrators (built-in)"));
        readerRoleObjectId = checkNotNull(ensureBuiltinRole(READER_ROLENAME, permissions.readerBasePermissions(), "Reader",
                          "Grants basic permissions for every cataloguemanager user (built-in)"));
    ...
    private String ensureBuiltinRole(String roleName,
        ...
        try {
            ...
            try {
                ...
                log.error("Unable to save fixed " + roleName + " role, please restart cataloguemanager to fix this.", e);
            }
        }

        if (previousRole == null) {
            log.error("Unable to access fixed " + roleName + " role, please restart cataloguemanager to fix this.");
            return null;
        }
    ...
    public RoleImpl save(Role role1) throws ValidationException {
        // sucky but necessary because of cataloguemanager2-shared not knowing about mongodb :(
