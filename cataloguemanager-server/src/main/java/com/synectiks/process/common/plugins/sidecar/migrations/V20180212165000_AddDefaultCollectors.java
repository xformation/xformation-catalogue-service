...
public class V20180212165000_AddDefaultCollectors extends Migration {
    ...
    public void upgrade() {
        ...
        final String beatsPreambel =
                "# Needed for cataloguemanager\n" +
                "fields_under_root: true\n" +
                "fields.collector_node_id: ${sidecar.nodeName}\n" +
                "fields.xfcatlg_source_collector: ${sidecar.nodeId}\n\n";
        ...
        ensureCollector(
                ...
                beatsPreambel +
                        ...
                        "path:\n" +
                        "  data: /var/lib/cataloguemanager-sidecar/collectors/filebeat/data\n" +
                        "  logs: /var/lib/cataloguemanager-sidecar/collectors/filebeat/log"
        );
        ensureCollector(
                "winlogbeat",
                "svc",
                "windows",
                "C:\\Program Files\\cataloguemanager\\sidecar\\winlogbeat.exe",
                "-c \"%s\"",
                "test config -c \"%s\"",
                beatsPreambel +
                        "output.logstash:\n" +
                        "   hosts: [\"192.168.1.1:5044\"]\n" +
                        "path:\n" +
                        "  data: C:\\Program Files\\cataloguemanager\\sidecar\\cache\\winlogbeat\\data\n" +
                        "  logs: C:\\Program Files\\cataloguemanager\\sidecar\\logs\n" +
                        "tags:\n" +
                        " - windows\n" +
                        "winlogbeat:\n" +
                        "  event_logs:\n" +
        ...
        ensureCollector(
                ...
                "define ROOT /usr/bin\n" +
                        ...
                        "\tOutputType  GELF_TCP\n" +
                        "\t<Exec>\n" +
                        "\t  # These fields are needed for cataloguemanager\n" +
                        "\t  $xfcatlg_source_collector = '${sidecar.nodeId}';\n" +
        ...
        ensureCollector(
                ...
                "svc",
                "windows",
                "C:\\Program Files\\cataloguemanager\\sidecar\\filebeat.exe",
                "-c \"%s\"",
                "test config -c \"%s\"",
                beatsPreambel +
                        "output.logstash:\n" +
                        "   hosts: [\"192.168.1.1:5044\"]\n" +
                        "path:\n" +
                        "  data: C:\\Program Files\\cataloguemanager\\sidecar\\cache\\filebeat\\data\n" +
                        "  logs: C:\\Program Files\\cataloguemanager\\sidecar\\logs\n" +
                        "tags:\n" +
                        " - windows\n" +
                        "filebeat.inputs:\n" +
    ...
    private String ensureCollector(String collectorName,
        ...
        try {
            ...
            try {
                ...
                LOG.error("Can't save collector " + collectorName + ", please restart cataloguemanager to fix this.", e);
            }
        }

        if (collector == null) {
            LOG.error("Unable to access fixed " + collectorName + " collector, please restart cataloguemanager to fix this.");
            return null;
        }
